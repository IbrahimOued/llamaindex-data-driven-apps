...
# Another important utility is `OnDemandLoaderTool`. This utility is designed to make the process
# of loading, indexing, and querying data seamless and efficient within an agent’s workflow, particularly
# when dealing with large volumes of data from various sources.

# It simplifies the process of using data loaders for agents by allowing them to trigger the loading,
# indexing, and querying of data through a single tool call.
# The normal approach in a RAG workflow would be to ingest all data at the start of our application, then
# chunk it, index it, and build a query engine on it. But that may not always be the most efficient method.

# It is especially useful in scenarios where data requirements are dynamic and
# unpredictable
# %%
from llama_index.core.agent.react import ReActAgent
...
# Next, let's define an on-demand tool for our agent, based on WikiperiaReader:
...
# Notice how I provided usage instructions in the description argument. These should help the agent
# better understand how to properly use the tool, although it might still take a few tries to get it right.
# Now, it's time to initialize the agent:

agent = ReActAgent.from_tools(tools=[tool], verbose=True, max_iterations=50)
response = agent.chat("What were some famous buildings in ancient Rome?")
print(response)
# %% [markdown]
# ### Using the LLMCompiler agent for more advanced scenarios

# While they tend to perform well in many scenarios, both OpenAI and ReAct agents have some
# drawbacks. Because current LLMs are not very good at long-term planning, they can sometimes get
# stuck in an infinite loop without finding the desired solution.

# But probably the biggest drawback of these types of agents is their serialized way of working. In other
# words, the execution of the steps is done in sequence. These agents wait for the output generated by
# one step to trigger the next step. This is a very inefficient approach in many practical scenarios. Often,
# a series of steps can be executed in parallel, significantly improving application performance and user
# experience. Based on these premises, I will now present a more advanced form of agent.
#
# The LLMCompiler agent orchestrates these parallel function calls using a three-part system that plans,
# dispatches, and executes tasks, resulting in faster and more accurate multi-function calls compared to
# sequential methods. Just as compilers transform and optimize code to run efficiently, LLMCompiler
# transforms natural language queries into optimized sequences of function calls that can be executed
# in parallel when dependencies allow
#
# Under the hood, an LLMCompileraAgent has three main components:
# * `LLM planner`: Formulates execution strategies and dependencies from user input and examples
# * `Task-fetching unit`: Sends and updates function-calling tasks based on the dependencies
# * `Executor`: Executes tasks in parallel using associated tools
# %% [markdown]
# The LLM planner determines the order of function calls and their interdependencies according to user
# input. Next, the task-fetching unit initiates parallel execution of these functions, replacing variables
# with the outputs from prior tasks. The executor then carries out these function calls with the relevant
# tools. Combined, these elements enhance the efficiency of parallel function calling in LLMs.
#
# The directed acyclic graph (DAG) of tasks is a key data structure created by the LLM planner from
# user inputs and examples. This planning graph captures task dependencies and enables optimized
#
# parallel execution (https://en.wikipedia.org/wiki/Directed_acyclic_graph).
# The DAG facilitates the simultaneous execution of tasks that do not depend on each other. Should
# one task rely on the completion of another, the prerequisite task must finish before the dependent task
# can commence. Independent tasks, on the other hand, are capable of being executed concurrently
...
# After importing LLMCompilerAgentPack and DatabaseToolSpec, we initialized the database
# tools and used the tool list to initialize the agent. It’s now time to interact with the agent, this time
