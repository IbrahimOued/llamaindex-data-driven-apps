...
def load_chat_store():
    try:
        load_chat_store = SimpleChatStore.from_persist_path(CONVERSATION_FILE)
    except FileNotFoundError:
        load_chat_store = SimpleChatStore()
...
def initialize_chatbot(user_name, study_subject, chat_store, container, context):
    memory = ChatMemoryBuffer.from_defaults(token_limit=3000, chat_store=chat_store, chat_store_key="0")
...
def chat_inerface(agent, chat_store, container):
    ...
    if prompt:
        with container:
            ...
            with st.chat_message("PITS"):
                st.markdown(response)
        chat_store.add_message("0", "user", prompt)